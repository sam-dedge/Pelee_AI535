...
def init_net(net, cfg, resume_net):
    '''if not cfg.model.init_net and not resume_net:
        print('Training ffrom _init__()', resume_net, net)
    '''
    if cfg.model.init_net and not resume_net:
        print('Training ffrom pretrained architecture')
...
def get_dataloader(cfg, dataset, setname='train_sets'):
    ...
    Dataloader_function = {'VOC': VOCDetection, 'COCO': None}
    ...
    print('Returning Dataset', type(dataset))
    return dataset
...
def save_checkpoint(net, cfg, final=True, datasetname='COCO', epoch=10):
    ...
    if final:
        torch.save(net.state_dict(), weights_save_path +
                   'Final_Pelee_{}_size{}_batchSize{}_gamma{}.pth'.format(datasetname, cfg.model.input_size, cfg.train_cfg.per_batch_size, cfg.train_cfg.gamma))
    else:
        torch.save(net.state_dict(), weights_save_path +
                   'Pelee_{}_size{}_epoch{}_batchSize{}_gamma{}.pth'.format(datasetname, cfg.model.input_size, epoch, cfg.train_cfg.per_batch_size, cfg.train_cfg.gamma))
